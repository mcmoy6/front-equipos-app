...
import { printerStartUpdateAction, uiCloseModalPrinterEditAction } from '../../actions/printersActions';
...
export const PrinterModalEdit = () => {


    const dispatch = useDispatch();

    const { modalOpenPrinterEdit } = useSelector(state => state.ui);

    const { activeRowPrinter } = useSelector( state => state.printers );
    ...
    useEffect(() => {
     if ( activeRowPrinter ) {
        setFormValues( activeRowPrinter );
     }    
    }, [activeRowPrinter, setFormValues]);

    const handleInputChange = ({target}) => {
        // console.log(target.value);
       setFormValues({

           ...formValues,//Se desestructura todo el values, para jalar todos los elementos que no estan cambiando
    ...
    const closeModal = () => {
        ...
        dispatch( uiCloseModalPrinterEditAction() );
    }
    ...
    const handleSubmitForm = (e) => {
        ...
        dispatch( printerStartUpdateAction( formValues ) );
    ...
    return (
        ...
        > 
        <h5> ACTUALIZAR DATOS IMPRESORA </h5>
        <hr />
